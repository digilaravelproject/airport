...
class InventoryController extends Controller
{
    ...
    /**
     * Capture an on-device screenshot via ADB and return a public URL.
     * Route (POST): inventories.screenshot
     */
    public function screenshot(Inventory $inventory)
    {
        // Must have an IP on the inventory
        $ip = $inventory->box_ip ?? null;
        if (!$ip) {
            return response()->json([
                'success' => false,
                'message' => 'No device IP on this inventory.',
            ], 422);
        }
        ...
        // Ensure adb keys location if needed (same pattern as your other controller)
        putenv('ADB_VENDOR_KEYS=/var/www/.android');

        $adb    = escapeshellarg($adbPath);
        $serial = escapeshellarg("{$ip}:5555");

        // 1) Ping first (quick fail if unreachable)
        $pingCmd = (stripos(PHP_OS, 'WIN') === 0) ? "ping -n 1 $ip" : "ping -c 1 $ip";
        exec($pingCmd, $pingOut, $pingStatus);
        if ($pingStatus !== 0) {
            return response()->json([
                'success' => false,
                'message' => 'Device not reachable (ping failed)',
                'raw'     => $pingOut,
                'code'    => 408,
            ], 408);
        }

        // 2) ADB connect (disconnect just in case)
        ...
        // 3) Build output path under storage/app/public/screenshots
        $dir = storage_path('app/public/screenshots');
        if (!is_dir($dir)) {
            @mkdir($dir, 0775, true);
        }
        // give each shot a timestamped filename to avoid browser cache
        $filename = 'inv_'.$inventory->id.'_'.time().'.png';
        $fullpath = $dir . DIRECTORY_SEPARATOR . $filename;

        // 4) Capture: exec-out screencap -p  -> write file directly (no PHP buffering)
        // NOTE: we run via sh -c so output redirection works cross-shell
        $cmd = "sh -c $adb -s $serial exec-out screencap -p > " . escapeshellarg($fullpath);
        exec($cmd, $shotOut, $shotStatus);

        // Basic sanity: file exists and has size
        if (!file_exists($fullpath) || filesize($fullpath) < 1000) {
            // try to include some diagnostic info
            return response()->json([
                'success' => false,
                'message' => 'Screenshot failed (no image produced).',
                'raw'     => $shotOut,
                'code'    => 500,
            ], 500);
        }

        // 5) Make a public URL (requires storage:link)
